...
FBT_TOOLCHAIN_VERSION="${FBT_TOOLCHAIN_VERSION:-"44"}";
...
fbtenv_check_sourced()
{
    ...
    case ${0##*/} in dash|-dash|bash|-bash|ksh|-ksh|sh|-sh|*.sh|fbt|ufbt|fbtng)
        return 0;;
    esac
    fbtenv_show_usage;
    return 1;
...
fbtenv_check_env_vars()
{
    ...
    if [ "$FBT_TOOLCHAIN_PATH_WAS_SET" -eq 0 ] && [ ! -x "$DEFAULT_SCRIPT_PATH/fbt" ] && [ ! -x "$DEFAULT_SCRIPT_PATH/ufbt" ] && [ ! -x "$DEFAULT_SCRIPT_PATH/fbtng" ] ; then
